...
Научио си већ да манипулишеш низовима на уобичајени начин, приступањем елементима
низа помоћу индекса. На пример, ако декларишемо низ a[10], то значи да смо
резервисали 10 меморијских локација којима приступамо једноставном петљом:
...
Елементима низа можеш приступити и помоћу показивача и то ћемо показати на
следећем примеру:
...
&a[i]⬄ a+i, адреса i+1 елемента низа, односно елемента са индексом i. Сходно
томе, ако у претходном изразу индексу i доделимо вредност нула, добијамо да
...
```{infonote}
!Обрати пажњу: Само име низа, у овом случају а, представља адресу првог елемента
...
Напомена: У називу задатка се налази линк ка задатку, а у загради страна на којој
се налази задатак у PDF формату збирке.
...
main()
{
	...
	printf("Izgled niza u obrnutom poretku\\n");
	for (i = n - 1;i >= 0;i--)
		printf("a[%d]= %d ", i, *(a + i));		
}
```
...
a[0]=23 a[1]=7 a[2]=14 a[3]=37 a[4]=52
Izgled niza u obrnutom poretku
a[4]= 52 a[3]= 37 a[2]= 14 a[1]= 7 a[0]= 23
```

```{infonote}
!Напомена: Ово је био типичан школски пример. У тест примерима због ограничења меморије и времена извршења задатка, програм  се своди на учитавање података са стандардног улаза и исписивање на стандардни излаз без „сувишног“ текста. 
```
